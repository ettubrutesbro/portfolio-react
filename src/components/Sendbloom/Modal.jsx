...
export default class Modal extends React.Component {
  ...
  componentDidMount = () => {
    ...
    makeColorMesh('sendbloom', this.refs.body, this.aptecrows.geometry, [
      //front
      { range: [42, 61], color: 0xededed },
      { range: [64, 69], color: 0xededed },
      //front inside
      { range: [32, 37], color: 0xfbfbfc },
      { faces: [40, 41], color: 0xfbfbfc },
      //inside sides
      { range: [4, 7], color: 0xccd2d6 },
      { range: [10, 13], color: 0xccd2d6 },
      { range: [18, 21], color: 0xccd2d6 },
      { faces: [26, 27, 38, 39], color: 0xccd2d6 },
      //top and bottom inside
      { range: [14, 17], color: 0xe1e3e5 },
      { range: [22, 25], color: 0xe1e3e5 },
      { range: [28, 31], color: 0xe1e3e5 },
      { faces: [2, 3, 8, 9], color: 0xe1e3e5 },
      //sides, and back
      { range: [70, 73], color: 0xccd2d6 },
      { faces: [0, 1], color: 0xededed },
      //top and bottom
      { faces: [74, 75], color: 0xfbfbfc },
      { faces: [62, 63], color: 0xb5bfc4 },
    ])
    makeColorBox(
      'sendbloom',
    ...
    makeColorMesh('sendbloom', this.refs.aptecslot, this.aptecslot.geometry, [
      ...
      //face down
      { faces: [2, 3, 4, 5, 8, 9], color: 0xb5bfc4 },
      { range: [22, 33], color: 0xb5bfc4 },
      //outsides
      { faces: [54, 55], color: 0xfbfbfc },
      { faces: [39, 40], color: 0xccd2d6 },
      { faces: [52, 53, 58, 59], color: 0xededed },
      { range: [34, 38], color: 0xededed },
      { range: [41, 51], color: 0xededed },
    ])
    ...
    makeColorBox(
      'sendbloom',
      this.refs.aptecxleft,
      [1.0918, 0.123, 0.114],
      windowcolor
    )
    makeColorBox(
      'sendbloom',
      this.refs.aptecxright,
      [0.23, 0.123, 0.114],
      windowcolor
    )
    makeColorBox(
  ...
  mount = unselect => {
    ...
    let modalScale = [
      { x: modal.scale.x, y: modal.scale.y, z: modal.scale.z },
      { x: 1, y: unselect ? 0.01 : 1, z: 1 },
    ]

    const reset = unselect
      ? () => {
          modal.visible = false
    ...
    this.selectTweens = [
      twn('scale', modalScale[0], modalScale[1], 550, modal.scale, {
        onComplete: reset,
      }),
      twn(
        'opacity',
  ...
  cycleOut = () => {
    ...
    this.cycleTweens = [
      ...
      twn('scale', { y: content.scale.y }, { y: 0.001 }, 350, content.scale, {
        onComplete: () => {
          modal.visible = false
          this.switchBody(true)
        },
  ...
  switchBody = reset => {
    ...
    if (!reset) {
      ...
      this.bodyTweens = [
        ...
        twn('opacity', { opacity: 1 }, { opacity: 0 }, 425, list, {
          onComplete: () => {
            list.visible = false
          },
        ...
        //move / fade party
        twn('opacity', { opacity: 0 }, { opacity: 1 }, 425, partyobj, {
          onStart: () => {
            partyobj.visible = true
          },
          traverseOpacity: true,
        }),
        ...
        twn('scale', { x: topleft.scale.x }, { x: 0.8 }, 225, topleft.scale, {
          delay: 450,
        }),
        twn('scale', { x: topright.scale.x }, { x: 1 }, 225, topright.scale, {
          delay: 450,
        }),
        twn(
          'position',
          { z: xButton.position.z },
          { z: 0.03 },
          250,
          xButton.position,
          { delay: 650 }
      ...
      this.bodyTweens = [
        twn('opacity', { opacity: 0 }, { opacity: 1 }, 10, list, {
          onStart: () => {
            list.visible = true
          },
          traverseOpacity: true,
        }),
        twn('opacity', { opacity: 1 }, { opacity: 0 }, 10, partyobj, {
          onComplete: () => {
            partyobj.visible = false
          },
          traverseOpacity: true,
        }),
      ]
          ...
          <group ref="nonshadowcontents">
            <group ref="aptecbody">
              ...
              <mesh
                name="sendbloom"
                ref="modalbuttontext"
                position={v3(0.455, 0.175, 0.062)}
              >
                <planeBufferGeometry width={0.02} height={0.0175} />
                <meshBasicMaterial color={0xfbfbfc} transparent />
              </mesh>
            </group>
            <group ref="aptecxleft" position={v3(-0.089, 0.31, 0.0165)} />
            <group ref="aptecslot">
              <group ref="aptecx" />
            </group>
            <group position={v3(0.555, 0.31, 0.0165)}>
            ...
            <group ref="party" position={v3(1.15, -0.05, 0.015)}>
