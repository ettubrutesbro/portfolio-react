...
import {
  computeClipDifference,
  computeColorDifference,
  computeXformDifference,
  easings,
} from './helpers.js'
...
const projectNames = Projects.map(function(proj) {
  return proj.name
})
...
class App extends React.Component {
  ...
  componentDidMount() {
    window.addEventListener('scroll', () => {
      window.requestAnimationFrame(this.logScroll)
    })
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        {store.selectedProject && <ProjectInfo project={activeProject} />}
      </div>
    )
  }
}
...
@observer
class Moballax extends React.Component {
  ...
  @computed
  get percentage() {
    return store.scrollposition > this.props.yStart &&
    store.scrollposition < this.props.yEnd
      ? (store.scrollposition - this.props.yStart) /
        (this.props.yEnd - this.props.yStart)
      : store.scrollposition <= this.props.yStart
        ? 0
        : store.scrollposition >= this.props.yEnd ? 1 : null
  }
  @computed
  get interpolatedStyle() {
    ...
    const interpolatedValues = endValues.map((val, i) => {
      return allKeys[i] === 'opacity'
        ? (val - startValues[i]) * this.percentage
        : allKeys[i] === 'clipPath'
          ? computeClipDifference(
              startValues[i],
              val,
