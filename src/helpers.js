function computeChange(start, end, multiplier, easing) {
  ...
  return easing
    ? easing(multiplier, start, end - start, 1)
    : start + (end - start) * multiplier
}

export function computeXformDifference(
  startXform,
  endXform,
  percentage,
  easing,
  ...
  if (JSON.stringify(startKeys.sort()) !== JSON.stringify(endKeys.sort()))
    return

  let translation = { x: 0, y: 0 }
  if (endKeys.includes('x'))
    translation.x =
      computeChange(startXform.x, endXform.x, percentage, easing) + units
  if (endKeys.includes('y'))
    translation.y =
      computeChange(startXform.y, endXform.y, percentage, easing) + units
  translation = 'translate3d(' + translation.x + ', ' + translation.y + ', 0)'
  const rotation = endKeys.includes('rotation')
    ? 'rotate(' +
      computeChange(
        startXform.rotation,
        endXform.rotation,
      ...
      'deg)'
    : ''
  const scale = endKeys.includes('scale')
    ? 'scale(' +
      computeChange(startXform.scale, endXform.scale, percentage, easing) +
      ')'
    : ''
...
export function computeClipDifference(startPath, endPath, percentage, easing) {
  ...
  for (var i = 0; i < endPath.length; i++) {
    const xDifference = computeChange(
      startPath[i][0],
      endPath[i][0],
      percentage,
      easing || false
    )
    const yDifference = computeChange(
      startPath[i][1],
      endPath[i][1],
      percentage,
      easing || false
...
export function computeColorDifference(
  ...
  const red = Math.floor(
    computeChange(startColor[0], endColor[0], percentage, easing || false)
  )
  const green = Math.floor(
    computeChange(startColor[1], endColor[1], percentage, easing || false)
  )
  const blue = Math.floor(
    computeChange(startColor[2], endColor[2], percentage, easing || false)
  )
